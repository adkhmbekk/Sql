...
select CONCAT_WS('-', EMPLOYEE_ID, FIRST_NAME, LAST_NAME) from Employees
...
select FIRST_NAME, len(FIRST_NAME) as names_len from Employees
...
select Year1, case when Max1 > Max2 then Max1
...
SELECT 
    StudentID,
    -- FirstName: от начала до первого пробела
    SUBSTRING(FullName, 1, CHARINDEX(' ', FullName) - 1) AS FirstName,

    -- MiddleName: от первого пробела+1 до второго пробела
    SUBSTRING(
        FullName,
    ...
    -- LastName: от второго пробела+1 до конца
    SUBSTRING(
        FullName,
...
SELECT DEPARTMENT_ID,
       ...
       CAST(SUM(CASE WHEN DATEDIFF(YEAR, HIRE_DATE, GETDATE()) > 3 THEN 1 ELSE 0 END) * 100.0 / COUNT(*) AS DECIMAL(5,2)) AS PercentOver3Years
FROM Employees
...
SELECT JobDescription,
       MAX(CASE WHEN rn_desc = 1 THEN SpacemanID END) AS MostExperiencedID,
       MAX(CASE WHEN rn_asc = 1 THEN SpacemanID END) AS LeastExperiencedID
FROM RankedSpacemen
...
SELECT 
    STRING_AGG(CASE WHEN ch LIKE '[a-z]' THEN ch END, '') WITHIN GROUP (ORDER BY (SELECT 1)) AS Lowercase,
    STRING_AGG(CASE WHEN ch LIKE '[A-Z]' THEN ch END, '') WITHIN GROUP (ORDER BY (SELECT 1)) AS Uppercase,
    STRING_AGG(CASE WHEN ch LIKE '[0-9]' THEN ch END, '') WITHIN GROUP (ORDER BY (SELECT 1)) AS Numbers,
    STRING_AGG(CASE WHEN ch NOT LIKE '[a-zA-Z0-9]' THEN ch END, '') WITHIN GROUP (ORDER BY (SELECT 1)) AS Others
FROM Chars;
...
WHERE EXISTS (
    ...
    WHERE s2.Birthday = s1.Birthday AND s2.StudentName != s1.StudentName
)
